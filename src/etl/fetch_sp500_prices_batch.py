...
class SP500PriceFetcherBatch:
    ...
    def collect_data(self, symbols: List[str], delay: float = 0.5):
        ...
        logger.info(f"Resume mode: {len(done_symbols)} already done, {len(symbols_to_run)} remaining.")
        ...
        logger.info("=" * 60)
        logger.info(f"Successful fetches: {success}, Failed: {len(failed)}")
        if failed:
            logger.warning(f"Failed symbols: {failed[:10]}{' ...' if len(failed) > 10 else ''}")

        # load all temporary data and store in all_data
        self.load_tmp_data()

    def load_tmp_data(self):
        """Load all temporary Parquet files into all_data"""
        ...
        self.all_data = dfs
        logger.info(f"Loaded {len(dfs)} temporary files ({sum(len(df) for df in dfs)} records).")
        ...
        for year_dir in sorted(self.raw_dir.glob("year=*")):
            ...
            for month_dir in sorted(year_dir.glob("month=*")):
                ...
                logger.info(
                    ...
                    f"Symbols: {df.symbol.nunique()}, Records: {len(df)}"
                )

def main():
    fetcher = SP500PriceFetcherBatch(start_date="2017-01-01", end_date=None, data_dir="data")
